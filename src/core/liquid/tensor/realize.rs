    ...
    for (step, item) in plan.items.iter().enumerate() {
        match item {
            ExecItem::Kernel {
                ...
                let input_ptrs: Vec<*const u8> = inputs
                    ...
                    .map(|&buf_id| -> Result<*const u8> {
                        if let Some((bytes, _, _)) = realized_bytes.get(&buf_id) {
                            Ok(bytes.as_ptr())
                        } else if let Some(buf) = realized_buffers.get(&buf_id) {
                            Ok(buf.as_ptr_u8())
                        } else {
            ...
            ExecItem::ConstFill {
                ...
                realized_buffers.insert(*output, buffer);
            }
        }

        // Drop dead intermediates.
        for &node_id in &release_at[step] {
            let _ = realized_buffers.remove(&node_id);
            if let Some((bytes, dtype, numel)) = realized_bytes.remove(&node_id) {
                pool.lock().unwrap().release(dtype, numel, bytes);
            }
        }
    }

    let buf = if let Some(buf) = realized_buffers.remove(&plan.output) {
        buf
    } else if let Some((bytes, dtype, _)) = realized_bytes.remove(&plan.output) {
        exec::buffer_from_bytes(bytes, dtype)
    } else {
        return Err(anyhow!("Root node was not realized"));
    };
...
fn resolve_buffer(
    ...
    if let Some(buf) = realized_buffers.get(&node_id) {
        Ok(buf.clone())
    } else if let Some((bytes, dtype, _)) = realized_bytes.remove(&node_id) {
        let typed = exec::buffer_from_bytes(bytes, dtype);
        realized_buffers.insert(node_id, typed.clone());
        Ok(typed)
    } else {
