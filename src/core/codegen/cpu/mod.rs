...
use crate::core::codegen::generator::{CodeGenerator, FusedKernel, GeneratedKernel};
...
///
/// Implements the `CodeGenerator` trait for CPU targets.
/// Generates Cranelift IR that is compiled into native code.
...
impl CodeGenerator for CpuCodeGenerator {
    fn generate_kernel(
        ...
        let reduce_spec = kernel.reduce.as_ref().map(|r| {
            let kind = match r.op {
                ScheduleReduceKind::Sum => ReduceKind::Sum,
                ScheduleReduceKind::Prod => ReduceKind::Prod,
                ScheduleReduceKind::Max => ReduceKind::Max,
                ScheduleReduceKind::Min => ReduceKind::Min,
            };
