...
//! (`collect_kernel_inputs` in `liquid/schedule/fused_kernel.rs`).
//!
//! ## Contract Overview
//!
...
//!
//! ### Liquid Policy (Conservative)
//! - Multi-consumer nodes materialized → More buffer loads, less duplicate compute
//! - CSE memoization still valuable for shared subexpressions within single-consumer chains
//!
//! ### Solid Policy (Aggressive)
//! - Multi-consumer nodes inlined → Expression trees duplicated across consumers
//! - CSE memoization CRITICAL to prevent redundant IR building for same node
...
use crate::core::shared::codegen::math::{math_func_ref_for_op, MathFuncRefs};
