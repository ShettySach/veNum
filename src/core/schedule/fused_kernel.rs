...
pub(super) struct KernelInputCollector<'a> {
    ...
    pub input_set: &'a mut HashSet<NodeId>,
...
/// 2. **Multi-consumer nodes** (conservative policy): Materialized to avoid duplicate computation
...
pub(super) fn collect_kernel_inputs(
    ...
    for &input_id in &node.inputs {
        ...
        if can_inline {
            ...
            if collector.input_set.insert(input_id) {
